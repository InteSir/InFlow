      ...
      <div className="flex-1 flex items-center justify-between">
        <div className="flex-1">
          ...
          <div className="flex items-center">
            {isCurrent ? (
              <div
                className="bg-green-500/80 h-[20px] px-2 w-[81px] mt-2
              flex items-center justify-center text-xs text-white rounded-lg"
              >
                Active now
              </div>
            ):(
              <span className="mr-[16px] text-[12px] text-muted-foreground font-normal">{timeAgo}</span>
            )}
          </div>
        </div>

        {!isCurrent && (
          <Button variant="ghost" size="icon" disabled={loading} onClick={handleRemove}>
