...
import {isPast,formatDistanceToNowStrict,format} from "date-fns";
...
export const parseUserAgent = (userAgent:string,createdAt:string):AgentType=>{
    ...
    const formattedAt = isPast(new Date(createdAt)) ? `${formatDistanceToNowStrict(new Date(createdAt))} ago`
